...
serve(async (req) => {
  ...
  try {
    const { userId, campaignName, messageTemplate, subject, customerIds, sendToAll }: SendEmailRequest = await req.json();
    
    console.log('Email campaign request:', { campaignName, subject, customerCount: customerIds?.length || 'all' });
